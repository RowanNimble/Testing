pipeline {
                ...
                script {
                    ...
                    if (env.IS_PR_BUILD == "true") {
                        echo """
                            ...
                            This build will only validate (build package + check metadata).
                            Deploy and Run stages will be skipped — the branch build
                            already handled the DEV deploy.
                        """
                    }
                }
            }
    ...
    post {
        success {
            echo """
                Pipeline successful.
                Build type:     ${env.IS_PR_BUILD == 'true' ? 'PR validation' : 'Branch / merge'}
                Branch:         ${env.SOURCE_BRANCH}
